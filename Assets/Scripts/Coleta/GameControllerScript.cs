    ...
    public class GameControllerScript : MonoBehaviour
    {
        ...
        public void OnOptionChoose(TMP_Text opt)
        {
            ...
            if (totalGrabbedItems == totalItemsToGrab)
            {
                ...
                float endGameTime = timerController.GetComponent<TimerScript>().timeLeft;

                if (endGameTime <= 120 && endGameTime >= 90)
                {
                    score = 9000;
                }
                ...
                else if (endGameTime < 30 && endGameTime > 0)
                {
                    score = 1000;
                }

                ScoreManager.UpdateTotalScore(score);
                ...
                {
                    playerData = new PlayerData(currentPhase: 3, punctuation: ScoreManager.GetTotalScore());
                }
                else
                {
                    playerData = new PlayerData(currentPhase: SaveAndLoad.playerData.currentPhase, punctuation: ScoreManager.GetTotalScore());
                }

...
namespace Corrida
{
    public class PlayerCorrida : MonoBehaviour
    {
        ...
        [SerializeField][Min(0f)] private float jumpGravityScale = 3f;
        [SerializeField][Min(0f)] private float fallGravityScale = 16f;
        ...
        private void Update()
        {
            ...
            if (State == PlayerState.Running)
            {
                ...
                if (Input.touchCount > 0 && !EventSystem.current.IsPointerOverGameObject(0))
                {
                    ...
                    if (CanJump && phase == TouchPhase.Began)
                    {
                        isJumping = true;
                    }
                    if (isJumping && (phase == TouchPhase.Stationary || phase == TouchPhase.Moved))
                    {
        ...
        {
            // Alterar a gravidade do jogador quando pular
            if (isJumping && rb.gravityScale != jumpGravityScale)
            {
            ...
            if (!isJumping && (rb.velocity.y < 0f || jumpTimer == 0f) && rb.gravityScale != fallGravityScale)
            {
